...
@section ('content')
    <h2 class="title-bar no-border-bottom">
        {{ __("Availability Catering Equipment") }}
    </h2>
    <div class="language-navigation">
        <div class="panel-body">
            <div class="filter-div d-flex justify-content-between ">
                ...
                <div class="col-right">
                    @if($rows->total() > 0)
                        <span class="count-string">{{ __("Showing :from - :to of :total catering equipment",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    @else
        <div class="alert alert-warning">{{__("No catering equipment found")}}</div>
    @endif
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab',function (e) {
			...
			calendar = new FullCalendar.Calendar(calendarEl, {
				...
				events:{
                    	url:"{{route('catering.vendor.availability.loadDates')}}",
						extraParams:{
							id:lastId,
                        }
                },
        ...
        formModal = new Vue({
            ...
            methods:{
                ...
                saveForm:function () {
                    ...
                    $.ajax({
                        url:'{{route('catering.vendor.availability.store')}}',
                        data:this.form,
                        dataType:'json',
                        method:'post',
