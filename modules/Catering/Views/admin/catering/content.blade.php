<div class="panel">
    <div class="panel-title"><strong>{{__("Catering Content")}}</strong></div>
    <div class="panel-body">
        <div class="form-group">
            <label>{{__("Title")}}</label>
            <input type="text" value="{!! clean($translation->title) !!}" placeholder="{{__("Name of the catering equipment")}}" name="title" class="form-control">
        </div>
        <div class="form-group">
                ...
                @if(!empty($translation->faqs))
                    ...
                    @foreach($translation->faqs as $key=>$faq)
                        <div class="item" data-number="{{$key}}">
                            <div class="row">
                                <div class="col-md-5">
                                    <input type="text" name="faqs[{{$key}}][title]" class="form-control" value="{{$faq['title']}}" placeholder="{{__('Eg: When and where does the lease end?')}}">
                                </div>
                                <div class="col-md-6">
