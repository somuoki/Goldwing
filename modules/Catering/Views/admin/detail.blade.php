...
@section('content')
    <form action="{{route('catering.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        @csrf
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Edit: ').$row->title : __('Add new catering')}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url('catering' ) }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}" target="_blank">{{__("View Catering")}}</a>
                    @endif
                </div>
            </div>
            ...
            <div class="lang-content-box">
                <div class="row">
                    <div class="col-md-9">
                        @include('Catering::admin.catering.content')
                        @include('Catering::admin.catering.location')
                        @include('Catering::admin.catering.pricing')
                        @if(is_default_lang())
                            {{--@include('Catering::admin.catering.availability')--}}
                        @endif
                    ...
                    <div class="col-md-3">
                        ...
                        @if(is_default_lang())
                            <div class="panel">
                                ...
                                <div class="panel-body">
                                    <div class="form-group">
                                        <label>{{__('Catering Featured')}}</label>
                                        <br>
                                        <label>
                            ...
                            @include('Catering::admin.catering.attributes')
                            <div class="panel">
